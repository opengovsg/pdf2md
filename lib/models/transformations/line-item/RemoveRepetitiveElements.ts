...
import { isDigit } from '../../../util/string-functions'

interface ItemStore {
  minY: number
  maxY: number
  minElements: LineItem[]
  maxElements: LineItem[]
}

interface PageStoreItem {
  minElements: LineItem[]
  maxElements: LineItem[]
  minLineHash: number
  maxLineHash: number
...
export default class RemoveRepetitiveElements extends ToLineItemTransformation {
  ...
  transform(parseResult: ParseResult): ParseResult {
    ...
    const pageStore: PageStoreItem[] = []
    const minLineHashRepetitions: HashRepetitions = {}
    const maxLineHashRepetitions: HashRepetitions = {}

    parseResult.pages.forEach(page => {
      const items = page.items as unknown as LineItem[]
      const minMaxItems = items.reduce((itemStore: ItemStore, item) => {
